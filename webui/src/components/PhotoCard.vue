...
export default {
  ...
  methods: {
    ...
    async likePhoto() {
      try {
        await this.$axios.post(`/users/${this.photoOwner}/photos/${this.photoId}/likes/`, parseInt(localStorage.getItem('userid')), {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('userid')}`
          }
        })
...
<template>
  <div id="photo-card">
    ...
    <div id="comments-side" :style="{ height: commentSideHeight }">
      <div id="post-comment" style="margin-bottom: 10px; display: flex; flex-direction: row; align-content: baseline; min-width: 60%;">
        <input v-model="commentInput" @input="limitBytes" @keyup.enter="postComment" type="text" placeholder="Add a comment..." id="comment-input">
        <button id="post-comment-button" :disabled="commentInput ? false : true" @click="postComment">Post</button>
      </div>
      <div v-for="(comment, index) in comments" style="width: 100%; display: flex; flex-direction: row; justify-content: space-between; align-content: center;">
        <p style="margin: 5px; margin-left: 10px">{{ comment.user }}: {{ comment.content }}</p>
        <img v-if="comment.user === currentUser" @click="deleteComment(index)" src="\src\assets\red_delete_icon.png" alt="delete" style="cursor: pointer; margin-top: 5px; height: 16px; margin-right: 10px; color: red;">
      </div>
    </div>
  </div>
</template>
