...
export default {
	...
	methods: {
		onInputUpdate() {
			...
			this.disabled = !(bytes >= 3 && bytes <= 16);
		},
		async changeUsername() {
			...
			try {
				const response = await this.$axios.put(`/users/${localStorage.getItem('userid')}`, {
...
<template>
	<div class="change-username">
		...
		<form @submit.prevent="changeUsername">
			<input type="text" v-model="newUsername" @input="onInputUpdate"
